...
// 从响应中提取 token 用量（input_tokens / output_tokens）。
// 流式响应在 sse_events 的 message_start / message_delta 事件里；
// 非流式响应在 body.usage 里。
// Extract token usage (input_tokens / output_tokens) from the response.
// Streaming responses carry it in sse_events (message_start / message_delta);
// non-streaming responses carry it in body.usage.
function extractTokenUsage(data) {
  const resp = data?.response;
  if (!resp || typeof resp !== "object") return {};

  // 流式：遍历 sse_events 找 message_start（含 input_tokens）和 message_delta（含最终 output_tokens）。
  // Streaming: walk sse_events for message_start (has input_tokens) and message_delta (final output_tokens).
  if (Array.isArray(resp.sse_events)) {
    let inputTokens = 0;
    let outputTokens = 0;
    for (const evt of resp.sse_events) {
      let d = evt?.data;
      ...
      if (d.type === "message_start" && d.message && d.message.usage) {
        ...
        outputTokens = d.message.usage.output_tokens || 0;
      }
  ...
  // 非流式：直接读 body.usage。
  // Non-streaming: read body.usage directly.
  let body = resp.body;
