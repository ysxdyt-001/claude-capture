...
# 只抓 messages 相关请求（覆盖 v1/messages 和 v1/messages/batches 等）
TARGET_PATH_KEYWORD = "/messages"
...
def request(flow: http.HTTPFlow) -> None:
    ...
    if TARGET_PATH_KEYWORD in flow.request.path:
...
def response(flow: http.HTTPFlow) -> None:
    try:
        ...
        if TARGET_PATH_KEYWORD not in path:
            return
